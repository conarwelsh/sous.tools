...
import { ShoppingCart, CreditCard, ChevronRight, X, Loader2, ArrowLeft } from "lucide-react";
...
export const KioskFeature = () => {
  ...
  const { data, loading } = useQuery<KioskCatalogData>(GET_KIOSK_CATALOG, {
    variables: { orgId },
    skip: !orgId,
  });
  ...
  if (completed) {
    return (
      <View className="flex-1 items-center justify-center bg-zinc-950 p-8 text-center h-screen">
        <View className="mb-8 rounded-full bg-green-500/20 p-8">
          <Badge className="h-16 w-16 text-green-500" />
        </View>
        <h1 className="mb-4 text-6xl text-white font-black tracking-tighter uppercase">Enjoy!</h1>
        <Text className="mb-12 text-2xl text-zinc-400 font-medium">Your order has been sent to the kitchen.</Text>
        <Button size="lg" className="h-16 px-12 rounded-2xl text-xl" onClick={() => {
          setCompleted(false);
          setIsCheckout(false);
  ...
  if (isCheckout) {
    return (
      <View className="flex flex-col h-screen bg-zinc-950">
        ...
        <View className="flex-1 p-8 flex flex-row gap-12">
  ...
  return (
    <DevicePairingFlow type="kiosk">
      {() => (
        <View className="flex flex-col h-screen bg-[#050505] text-white">
          {/* Header */}
          ...
          <View className="flex flex-1 overflow-hidden">
            {/* Sidebar Categories */}
            <View className="w-80 bg-zinc-900/30 p-6 border-r border-zinc-800">
              <h3 className="text-xs font-black uppercase text-zinc-600 mb-8 px-4 tracking-[0.2em]">Menu Categories</h3>
              <ScrollView className="gap-4">
                <Button
                  variant={selectedCategoryId === null ? "default" : "ghost"}
                  className={`w-full h-16 justify-start mb-4 rounded-2xl px-6 text-lg font-bold uppercase tracking-tight ${selectedCategoryId === null ? "bg-primary" : "text-zinc-500 hover:text-white"}`}
                ...
                {categories.map((cat: any) => (
                  <Button
                    ...
                    className={`w-full h-16 justify-start mb-4 rounded-2xl px-6 text-lg font-bold uppercase tracking-tight ${selectedCategoryId === cat.id ? "bg-primary" : "text-zinc-500 hover:text-white"}`}
            ...
            {/* Main Content */}
            <View className="flex-1 overflow-y-auto p-12">
               <View className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                 {filteredItems.map((product: any) => (
                   <Card 
                    key={product.id} 
                    className="bg-zinc-900/50 border-zinc-800 rounded-[2.5rem] overflow-hidden flex flex-col h-[450px] group hover:border-primary/50 transition-all cursor-pointer"
                    onClick={() => addToCart(product)}
                   >
                     <View className="h-56 bg-zinc-800/50 flex items-center justify-center relative overflow-hidden">
                        <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                        <Text className="text-7xl group-hover:scale-110 transition-transform duration-500">🍔</Text>
                     </View>
                     <View className="p-8 flex flex-col flex-1">
                       ...
                       <Text className="text-zinc-500 text-base line-clamp-3 mb-8 flex-1">Delicious freshly prepared {product.name.toLowerCase()} made with the finest local ingredients.</Text>
                       <View className="flex flex-row items-center justify-between mt-auto">
                          <Text className="text-2xl font-mono font-black text-primary">${(product.price / 100).toFixed(2)}</Text>
                          <Button className="rounded-xl h-12 px-6 font-black uppercase tracking-tighter">
                            Add
                          </Button>
                       </View>
                     </View>
