...
import { View, Text, Button, Card, Badge, ScrollView, Logo, KioskLoading, cn } from "@sous/ui";
import { DevicePairingFlow } from "../hardware/components/DevicePairingFlow";
import { ShoppingCart, CreditCard, ChevronRight, X, Loader2, ArrowLeft, CheckCircle2, Info, Grid, Plus } from "lucide-react";
    ...
    return (
      <View className="flex-1 items-center justify-center bg-[#050505] p-8 text-center h-screen relative overflow-hidden">
        ...
        >
          ...
          <h1 className="mb-6 text-9xl text-white font-black tracking-tighter uppercase italic">Enjoy!</h1>
          <Text className="mb-16 text-3xl text-zinc-500 font-bold uppercase tracking-widest">Order sent to the kitchen.</Text>
          <Button size="lg" className="h-24 px-20 rounded-[3rem] text-3xl font-black uppercase tracking-widest bg-primary text-black shadow-[0_0_50px_rgba(var(--primary),0.4)] transition-all hover:scale-105 active:scale-95" onClick={() => {
            setCompleted(false);
            setIsCheckout(false);
    ...
    return (
      <View className="flex flex-col h-screen bg-zinc-950">
        <View className="p-12 border-b border-zinc-800/50 flex flex-row items-center bg-black/40 backdrop-blur-xl">
          ...
          >
            <ArrowLeft size={32} />
            <span className="text-xl font-black uppercase tracking-widest">Back to Menu</span>
          </button>
          <div className="h-12 w-px bg-zinc-800 mx-4" />
          <h2 className="text-4xl text-white font-black uppercase tracking-tighter italic">Your Tray</h2>
        </View>
        ...
        <View className="flex-1 p-12 flex flex-row gap-12 overflow-hidden bg-[radial-gradient(var(--color-border)_1px,transparent_1px)] [background-size:40px_40px] bg-opacity-5">
          <View className="flex-1 bg-zinc-900/40 backdrop-blur-md rounded-[3rem] p-12 border border-zinc-800/50 flex flex-col shadow-2xl">
            <h3 className="text-sm text-zinc-500 mb-12 font-black uppercase tracking-[0.4em]">Tray Contents</h3>
            <ScrollView className="flex-1 pr-4">
              {cart.map(item => (
                ...
                >
                  <View>
                    <Text className="text-white text-3xl font-black uppercase tracking-tighter group-hover:text-primary transition-colors">{item.name}</Text>
                    <div className="flex items-center gap-4 mt-2">
                      <span className="text-primary font-black font-mono text-lg">x{item.quantity}</span>
                      <div className="h-1 w-8 bg-zinc-800 rounded-full" />
                      <span className="text-zinc-500 font-bold uppercase tracking-widest text-xs">Standard Serving</span>
                    </div>
                  </View>
                  <Text className="text-white font-mono text-3xl font-black">${((item.price * item.quantity) / 100).toFixed(2)}</Text>
                </motion.div>
              ))}
            </ScrollView>
            <View className="mt-12 pt-12 border-t-4 border-double border-zinc-800 flex flex-row justify-between items-end">
              <div>
                <span className="text-xs font-black text-zinc-600 uppercase tracking-[0.4em] block mb-2">Total Amount</span>
                <h2 className="text-5xl text-white font-black uppercase tracking-tighter italic">Order Total</h2>
              </div>
              <h2 className="text-7xl text-primary font-black font-mono drop-shadow-[0_0_20px_rgba(var(--primary),0.4)]">${(total / 100).toFixed(2)}</h2>
            </View>
          </View>
          ...
          <View className="w-[500px] bg-zinc-900/60 backdrop-blur-xl rounded-[3rem] p-16 flex flex-col justify-center items-center border border-zinc-800/50 shadow-2xl relative overflow-hidden">
             ...
             <h3 className="text-4xl text-white mb-6 font-black uppercase tracking-tighter italic text-center">Ready for Payment</h3>
             <Text className="text-zinc-500 text-center mb-16 text-xl leading-relaxed font-medium">Please Insert, Swipe or Tap your card on the terminal below.</Text>
             ...
             <Button 
               ...
               disabled={processing}
             >
               {processing ? <Loader2 className="animate-spin size-10" /> : "Complete Order"}
             </Button>
      ...
      <View className="h-32 border-b border-white/5 flex flex-row items-center justify-between px-16 bg-zinc-950/40 backdrop-blur-3xl sticky top-0 z-20 shadow-2xl relative">
        <View className="flex flex-row items-center gap-10">
           ...
           <div className="flex flex-col">
             <span className="text-[10px] font-black uppercase tracking-[0.5em] text-zinc-500 mb-1.5 leading-none">Automated Service Node</span>
             <h1 className="text-4xl font-black uppercase tracking-tighter leading-none italic text-white">
               Self-Service <span className="text-sky-500 drop-shadow-[0_0_15px_rgba(14,165,233,0.5)]">Station</span>
             </h1>
           </div>
        </View>
        ...
        {cart.length > 0 && (
          ...
          >
            ...
            >
              ...
              <ShoppingCart className="mr-5 h-9 w-9 drop-shadow-[0_0_10px_white]" strokeWidth={3} />
              <span className="text-3xl font-black uppercase tracking-tighter italic mr-8">View My Tray</span>
        ...
        <View className="w-[360px] bg-zinc-950/20 backdrop-blur-xl border-r border-white/5 flex flex-col">
          <View className="p-14 flex-1 flex flex-col">
            <div className="flex items-center gap-4 mb-12 opacity-60">
               ...
               <h3 className="text-[11px] font-black uppercase text-zinc-400 tracking-[0.5em]">Explore Menu</h3>
            </div>
            <ScrollView className="flex-1 -mx-6 px-6 no-scrollbar">
              <View className="gap-8 pb-16">
                ...
                >
                  ...
                  {selectedCategoryId === null && (
                    <motion.div layoutId="active-cat" className="absolute right-8 top-1/2 -translate-y-1/2 z-10">
                      <ChevronRight size={32} className="drop-shadow-[0_0_10px_white]" />
                    </motion.div>
                  )}
                </button>
            ...
            <div className="mt-auto pt-10 border-t border-white/5">
               <Button variant="ghost" className="w-full h-20 rounded-3xl text-zinc-600 hover:text-sky-400 gap-4 border border-transparent hover:border-sky-500/20 hover:bg-sky-500/5 transition-all duration-500">
                  <Info size={24} />
                  <span className="text-xs font-black uppercase tracking-[0.3em]">Need Assistance?</span>
               </Button>
            </div>
          </View>
        </View>
                   ...
                   >
                     ...
                     >
                       ...
                       <View className="h-80 bg-zinc-800/20 flex items-center justify-center relative overflow-hidden">
                          ...
                          <Text className="text-[10rem] group-hover:scale-110 group-hover:rotate-6 transition-all duration-[2s] ease-out drop-shadow-[0_20px_40px_rgba(0,0,0,0.5)]">🍔</Text>
                          ...
                          <div className="absolute bottom-10 right-10 opacity-0 group-hover:opacity-100 transition-all translate-y-6 group-hover:translate-y-0 duration-700">
                             <div className="bg-sky-500 p-6 rounded-[2rem] shadow-[0_0_30px_rgba(14,165,233,0.6)] neon-glow">
                                <Plus size={40} className="text-white drop-shadow-[0_0_5px_white]" strokeWidth={4} />
                             </div>
                          </div>
                       </View>
                       ...
                       <View className="p-14 flex flex-col flex-1 relative z-10">
                         <div className="flex flex-col gap-2 mb-6">
                           <span className="text-[10px] font-black uppercase tracking-[0.5em] text-zinc-500 group-hover:text-sky-500 transition-colors duration-500">Premium Curation</span>
                           <h3 className="text-5xl font-black uppercase tracking-tighter text-white italic leading-none group-hover:text-sky-400 transition-all duration-500">{product.name}</h3>
                         </div>
                         <Text className="text-zinc-400 text-xl line-clamp-3 mb-12 flex-1 font-medium leading-relaxed group-hover:text-zinc-300 transition-colors duration-500">
                           Hand-crafted {product.name.toLowerCase()} prepared fresh using premium local ingredients. 100% Chef-certified.
                         </Text>
                         <View className="flex flex-row items-end justify-between mt-auto">
                            <div className="flex flex-col">
                               <span className="text-[11px] font-black uppercase text-zinc-600 tracking-[0.4em] mb-2">Service Total</span>
                               <Text className="text-5xl font-mono font-black text-sky-500 drop-shadow-[0_0_15px_rgba(14,165,233,0.4)] tracking-tighter italic">${(product.price / 100).toFixed(2)}</Text>
                            </div>
                            <div className="h-24 w-24 rounded-[2.5rem] border-4 border-white/5 flex items-center justify-center group-hover:border-sky-500/50 group-hover:scale-110 group-hover:rotate-12 transition-all duration-700 bg-white/5">
                               <Plus size={32} className="text-zinc-700 group-hover:text-sky-400 group-hover:drop-shadow-[0_0_10px_rgba(56,189,248,0.5)] transition-all" strokeWidth={3} />
                            </div>
                         </View>
                       </View>
                     </Card>
