...
export function RecipeForm({
  ...
  const [formData, setFormData] = useState({
    ...
    ingredients:
      initialData?.ingredients?.map((ri: any) => ({
  ...
  const addStep = () => {
    ...
    setFormData({
      ...
      steps: [
        ...formData.steps,
        { order: nextOrder, instruction: "", timerDuration: "" },
      ],
    });
  };
    ...
    try {
      const payload = {
        ...
        yieldAmount: formData.yieldAmount
          ? parseFloat(formData.yieldAmount)
          : null,
        ...
        <View className="flex-row gap-4">
          ...
          <View className="flex-1 gap-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, yieldUnit: e.target.value })
              }
              placeholder="grams"
              className="h-10 bg-muted/50 border-border"
            />
      ...
      <View className="gap-4">
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center gap-2">
            ...
            <Text className="font-black uppercase text-sm tracking-widest">
              Ingredients
            </Text>
          </View>
          <Button
            ...
            className="h-8 border-primary/20 hover:bg-primary/5 gap-2"
          >
        ...
        <View className="gap-3">
          {formData.ingredients.map((ing: any, idx: number) => (
            <Card
              key={idx}
              className="p-3 bg-muted/20 border-border flex-row items-center gap-3"
            >
                ...
                <View className="flex-row items-center justify-between border-t border-border/30 pt-3">
                  <View className="flex-row items-center gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateStep(idx, { timerDuration: e.target.value })
                      }
                  ...
                  <Button
                    ...
                    className="h-8 px-2 hover:bg-destructive/10 hover:text-destructive gap-2 opacity-0 group-hover:opacity-100 transition-opacity"
                  >
                    <Trash2 size={12} />
