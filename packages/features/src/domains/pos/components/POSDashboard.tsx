    ...
    <View className="flex-1 p-10 bg-[#050505] overflow-y-auto relative">
      ...
      <div className="flex items-center justify-between mb-12 relative z-10">
        <div className="flex flex-col">
          <span className="text-[10px] font-black uppercase tracking-[0.5em] text-zinc-500 leading-none mb-2">
            Operational Analytics
          </span>
          <h1 className="text-4xl font-black uppercase tracking-tighter text-white italic">
            Shift{" "}
            <span className="text-sky-500 drop-shadow-[0_0_15px_rgba(14,165,233,0.4)]">
              Dashboard
            </span>
          </h1>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10 relative z-10">
        <Card className="bg-zinc-950/40 backdrop-blur-xl border-white/5 p-8 rounded-[3rem] h-[500px] flex flex-col shadow-2xl group hover:border-white/10 transition-all duration-500">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <h3 className="font-black uppercase tracking-[0.2em] text-white text-xs">
                Recent Velocity
              </h3>
            </div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto pr-2 no-scrollbar">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="flex justify-between items-center p-5 bg-white/5 border border-white/5 rounded-[2rem] group/item hover:bg-white/10 hover:border-white/10 transition-all duration-500"
              >
                <div className="flex gap-5 items-center">
                  <div className="w-12 h-12 rounded-2xl bg-black/40 flex items-center justify-center text-sky-400 font-black text-sm border border-white/5">
                    #{40 + i}
                  </div>
                  <div>
                    <div className="text-white font-black text-sm uppercase tracking-tight italic">
                      Table {i * 2}
                    </div>
                    <div className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mt-1">
                      Processed {i * 5}m ago
                    </div>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end">
                  <div className="text-white font-mono font-black text-lg">
                    ${(120.5 + i * 15).toFixed(2)}
                  </div>
                  <div className="text-emerald-500 text-[9px] font-black uppercase tracking-[0.2em] bg-emerald-500/10 px-2 py-0.5 rounded-full mt-1 border border-emerald-500/20">
                    Verified
                  </div>
                </div>
              </div>
        ...
        <Card className="bg-zinc-950/40 backdrop-blur-xl border-white/5 p-8 rounded-[3rem] h-[500px] flex flex-col shadow-2xl group hover:border-white/10 transition-all duration-500">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-[10px] font-black uppercase text-zinc-500 bg-black/40 px-4 py-1.5 rounded-full border border-white/5 shadow-inner italic">
              3 Inactive Items
            </span>
          </div>
          <div className="flex-1 space-y-4 overflow-y-auto pr-2 no-scrollbar">
            {[
              ...
              <div
                key={i}
                className="flex justify-between items-center p-5 bg-red-500/5 border border-red-500/10 rounded-[2rem] group/item hover:bg-red-500/10 transition-all duration-500"
              >
                <div className="flex items-center gap-5">
                  <div className="w-12 h-12 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-500 font-black text-sm border border-red-500/20">
                    !
                  </div>
                  <div>
                    <div className="text-white font-black text-sm uppercase tracking-tight italic">
                ...
                <button className="h-10 px-5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] bg-white/5 text-zinc-400 hover:text-white hover:bg-white/10 transition-all">
                  Resolve
                </button>
              </div>
            ))}
...
const MetricCard = ({ label, value, icon: Icon, color, glow, bg }: any) => (
  ...
  >
    ...
    <div className="flex justify-between items-start z-10 relative">
      <div
        className={cn(
          "p-4 rounded-2xl bg-black/40 border border-white/5 transition-all duration-500 group-hover:scale-110",
          color,
        )}
      >
        <Icon size={22} className="drop-shadow-[0_0_8px_currentColor]" />
      </div>
      <span className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 group-hover:text-zinc-300 transition-colors duration-500">
        {label}
      </span>
    </div>
    ...
    <div
      className={cn(
        "text-5xl font-black tracking-tighter z-10 relative italic drop-shadow-[0_0_10px_rgba(0,0,0,0.5)]",
        color,
      )}
    >
      {value}
    ...
    <div
      className={cn(
        "absolute -right-10 -bottom-10 w-40 h-40 rounded-full blur-[60px] opacity-20 transition-all duration-700 group-hover:opacity-40 group-hover:scale-125",
        bg.replace("bg-", "bg-").replace("/5", ""),
      )}
    />
  </div>
);
