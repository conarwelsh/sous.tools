...
export const POSSidebar = ({ activeTab, onTabChange, user, onLogout, onLogin }: POSSidebarProps) => {
  const tabs = [
  ...
  return (
    <View className="w-24 bg-zinc-950/50 backdrop-blur-xl border-r border-white/5 flex flex-col items-center py-8 justify-between z-20">
              ...
              >
                ...
                <Icon 
                  ...
                  className={cn(
                    ...
                    isActive ? "scale-110 drop-shadow-[0_0_8px_rgba(56,189,248,0.5)]" : "group-hover:scale-105"
                  )} 
                  strokeWidth={isActive ? 2.5 : 2}
                />
                <span className="text-[8px] font-black uppercase tracking-[0.2em] z-10">{tab.label}</span>
      ...
      <div className="flex flex-col gap-4 w-full px-2">
        ...
        >
          <Settings size={20} className={cn("transition-transform duration-500", activeTab === "settings" ? "rotate-90" : "group-hover:rotate-45")} />
          <span className="text-[8px] font-black uppercase tracking-[0.2em]">Setup</span>
        </button>
        ...
        {user ? (
          ...
          >
            ...
            <LogOut size={20} className="hidden group-hover:block animate-in zoom-in duration-300" />
