...
import { Loader2, Plus, Minus, Trash2, Send, CreditCard, ChevronRight } from "lucide-react";
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <h1 className="text-2xl font-black uppercase tracking-tighter italic text-white leading-none mt-1">
              Cart <span className="text-sky-500 drop-shadow-[0_0_10px_rgba(14,165,233,0.3)]">Manifest</span>
            </h1>
          </div>
          <Button variant="ghost" size="icon" className="text-zinc-600 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all" onClick={onClear}>
            <Trash2 size={18} />
          </Button>
        </div>
        ...
        <AnimatePresence initial={false}>
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-zinc-700 py-32 gap-6 opacity-40">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.4em] text-center">Manifest Awaiting<br/>Inbound Items</span>
            </div>
          ) : (
              ...
              {items.map((item) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col">
                      <span className="font-black text-white text-[13px] uppercase tracking-tight italic group-hover:text-sky-400 transition-colors">{item.name}</span>
                      <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Standard Option</span>
                    </div>
      ...
      <div className="p-8 pt-6 bg-black/40 backdrop-blur-3xl border-t border-white/5 space-y-8 relative z-10 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
        <div className="space-y-3">
          <div className="flex justify-between items-center text-[10px] px-1">
            <span className="text-zinc-500 font-black uppercase tracking-[0.2em]">Subtotal</span>
            <span className="text-zinc-300 font-mono font-bold">${(subtotal / 100).toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-center text-[10px] px-1 pb-4 border-b border-white/5">
            <span className="text-zinc-500 font-black uppercase tracking-[0.2em]">Tax Service (8%)</span>
            <span className="text-zinc-300 font-mono font-bold">${(tax / 100).toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-end pt-4 px-1">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-sky-500 uppercase tracking-[0.4em] mb-1">Due Amount</span>
              <span className="text-lg font-black text-white uppercase tracking-[0.2em] leading-none italic">Order Total</span>
            </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            <span className="font-black uppercase tracking-[0.25em] text-[10px]">Send to Kitchen</span>
          </Button>
          ...
          >
            ...
            <CreditCard size={22} className="mr-4 group-hover:scale-110 transition-transform" />
            <span className="font-black uppercase tracking-[0.2em] text-sm italic">Initialize Checkout</span>
          </Button>
        </div>
      </div>
    </div>
