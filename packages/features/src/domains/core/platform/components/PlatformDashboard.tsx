...
import { Card, CardHeader, CardTitle, CardContent, View, Text } from "@sous/ui";
import {
  Globe,
  Users,
  ShoppingCart,
...
const HealthItem = ({
  ...
  <View className="flex flex-row items-center justify-between py-2 border-b border-border/20 last:border-0">
    ...
    <View className="text-right">
      <Text className={`text-xs font-black uppercase tracking-widest ${color}`}>
        {status}
      </Text>
      {uptime && (
        <Text className="text-[8px] text-muted-foreground uppercase">
          {uptime} Uptime
        </Text>
      )}
    </View>
