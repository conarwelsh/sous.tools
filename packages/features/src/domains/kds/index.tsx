...
const KDSContent = ({ orgId }: { orgId: string }) => {
  ...
  const { data: subData } = useSubscription<{ orderUpdated: any }>(
    ORDER_SUBSCRIPTION,
    {
      skip: !orgId,
    },
      ...
      <View className="h-24 border-b border-white/5 bg-zinc-950/50 backdrop-blur-2xl flex flex-row items-center justify-between px-10 relative z-10 shadow-2xl">
        <View className="flex flex-row items-center gap-6">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500 leading-none mb-1.5">
              Kitchen Operation Center
            </span>
            <h1 className="text-2xl font-black uppercase tracking-tighter text-white italic leading-none">
              Kitchen Display{" "}
              <span className="text-sky-500 drop-shadow-[0_0_10px_rgba(14,165,233,0.4)]">
                System
              </span>
            </h1>
          </div>
        ...
        <View className="flex flex-row items-center gap-8">
          <div className="flex flex-row bg-black/40 p-1.5 rounded-2xl border border-white/5 backdrop-blur-md">
            ...
            >
              <List
                size={16}
                className={!showCompleted ? "drop-shadow-[0_0_5px_white]" : ""}
              />
              Active Tickets
            </button>
            <button
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              size="icon"
              onClick={() => refetch()}
              className="w-12 h-12 rounded-[1.25rem] text-zinc-500 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/5 transition-all group"
            >
              <RefreshCcw
                size={22}
                ...
                >
                  <OrderTicket
                    order={{
                      ...order,
                      number:
                        order.externalOrderId?.split("-").pop() ||
          ...
          <div className="p-12 space-y-10">
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 border border-white/5 p-6 rounded-[2rem] flex flex-col gap-2 group hover:border-sky-500/30 transition-all">
                    <span className="text-zinc-500 font-black uppercase text-[9px] tracking-widest">
                      Font Density
                    </span>
              ...
              <div className="space-y-4">
                ...
                <Button
                  variant="outline"
                  className="w-full h-16 rounded-[1.5rem] border-white/5 bg-white/5 text-zinc-400 hover:text-red-400 hover:border-red-500/20 hover:bg-red-500/5 transition-all duration-500 gap-4 group"
                >
                  <Trash2
                    size={18}
            ...
            <div className="pt-4">
              <Button
                className="w-full h-20 rounded-[2rem] bg-sky-500 hover:bg-sky-400 text-white font-black uppercase tracking-[0.25em] text-xs italic shadow-xl shadow-sky-500/20 transition-all duration-500"
