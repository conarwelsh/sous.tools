...
export const LabelEditor = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this label template?")) return;
    try {
      const http = await getHttpClient();
      ...
      {isLoading ? (
        ...
        <Card className="p-20 bg-card border-border border items-center justify-center">
          ...
          <Text className="text-muted-foreground text-sm font-medium mb-8 max-w-sm text-center">
            Create thermal label templates with dynamic data binding for expiration dates and barcodes.
          </Text>
              ...
              <div className="flex flex-row justify-between items-start mb-6">
                <View className="flex-1">
                  ...
                  <Text className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">
                    Thermal {layout.config?.dimensions?.width || 50}x{layout.config?.dimensions?.height || 30} Template
                  </Text>
                </View>
              </div>

              <div className="flex flex-row gap-2">
                <Button onClick={() => setEditingLayout(layout)} className="flex-1 h-10 bg-muted hover:bg-muted/80">
                  <View className="flex-row items-center gap-2">
