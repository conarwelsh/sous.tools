...
function SlotToolbar({ onEditStyle, onEditData, showData = true }: { onEditStyle: () => void, onEditData: () => void, showData?: boolean }) {
  return (
    <div className="absolute top-2 right-2 flex flex-row gap-1 opacity-0 group-hover/slot:opacity-100 transition-opacity z-[100]">
      ...
      >
        ...
        <span className="text-[9px] font-black uppercase tracking-tight">Style</span>
      </Button>
      {showData && (
        <Button 
          size="sm" 
        ...
        >
          ...
          <span className="text-[9px] font-black uppercase tracking-tight">Data</span>
        </Button>
      )}
    </div>
  );
    ...
    <View className="flex flex-col">
      ...
      >
        ...
        <div className="flex flex-row items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button 
            onClick={(e) => { e.stopPropagation(); onEdit(node, 'style'); }}
...
function DroppableNodeWrapper({ node, children, onNodeClick, isSelected, activeLayoutType, onEditStyle, onEditData }: any) {
...
function DesignerRenderer({ node, parentNode, onNodeClick, onEditClick, onResize, selectedNodeId, isRoot, contentMap, activeLayoutType }: any) {
  const isSelected = selectedNodeId === (node as any)._internalId;
  const isFixed = node.type === 'fixed';
...
export function LayoutDesigner({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log(`[LayoutDesigner] Fetching data... (Token: ${http.token ? 'YES' : 'NO'})`);
        ...
        if (results[0].status === 'rejected') console.error("[LayoutDesigner] Categories fetch failed:", results[0].reason);
        if (results[1].status === 'rejected') console.error("[LayoutDesigner] Products fetch failed:", results[1].reason);
        if (results[2].status === 'rejected') console.error("[LayoutDesigner] Hardware fetch failed:", results[2].reason);
    ...
    return (
      <View className="gap-6 mt-2">
        <View className="gap-2">
          <Text className="text-muted-foreground font-black uppercase text-[8px] tracking-widest">Source Provider</Text>
        ...
        {assignment.sourceType === 'POS' && (
          <View className="gap-6">
             <View className="gap-4">
                <Text className="text-muted-foreground font-black uppercase text-[8px] tracking-widest">Browse & Select Items</Text>
                ...
                <View className="gap-2">
                  ...
                  {(assignment.dataConfig?.filters?.itemIds || []).length > 0 && (
                    <Button 
                      variant="ghost" 
                      onClick={() => handleUpdateSlotAssignment(node.id!, {
                        dataConfig: { ...assignment.dataConfig, filters: { ...assignment.dataConfig?.filters, itemIds: [] } }
                      })}
                      className="h-6 self-end"
                    >
                      <span className="text-[7px] font-black uppercase text-sky-500 underline underline-offset-4">Clear selection</span>
                    </Button>
                  )}
                </View>
             </View>
             ...
             <View className="gap-4 border-t border-border pt-6">
                  ...
                  {[1, 2, 3, 4, 5, 6].map(num => (
                    <Button
                      ...
                      className={cn(
                        ...
                        (assignment.componentProps?.columns || 2) === num && "border-sky-500 bg-sky-500/5 ring-1 ring-sky-500/50 shadow-[0_0_15px_rgba(14,165,233,0.1)]"
                      )}
                    >
                      <span className={cn(
                ...
                <div className="flex flex-row gap-4">
                  ...
                  >
                    <Checkbox checked={assignment.componentProps?.showPrice !== false} />
                    <span className="text-[8px] font-black uppercase text-muted-foreground group-hover:text-foreground">Show Price</span>
                  </div>
                  ...
                  <div 
                    ...
                    className="flex items-center gap-2 cursor-pointer group"
                  >
                    <Checkbox checked={assignment.componentProps?.showDescription !== false} />
                    <span className="text-[8px] font-black uppercase text-muted-foreground group-hover:text-foreground">Show Description</span>
                  </div>
                </div>
             </View>
          </View>
          ...
          acc[id] = (
            <View className="flex-1 items-center justify-center p-8 bg-sky-500/5 border border-sky-500/10 m-4 rounded-3xl">
              ...
              <Text className="text-[10px] text-sky-500/40 font-black uppercase tracking-widest text-center">
                {itemIds.length > 0 ? 'Selected items not found' : categoryId ? 'No items in category' : 'Bind data in properties'}
              </Text>
            </View>
          );
        }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.data.current?.isNew) {
      ...
      const effectiveTargetId = (newNode.type === 'fixed') 
  ...
  return (
    ...
    >
      <View className="flex-1 h-screen bg-background flex-row overflow-hidden">
            ...
            <View className="p-6 gap-8">
              ...
              <View className="gap-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <DraggablePaletteItem type="container" icon={Grid3X3} label="Grid" data={{ isGrid: true }} />
                  <DraggablePaletteItem type="slot" icon={Square} label="Slot" />
                  <DraggablePaletteItem type="fixed" icon={Maximize} label="Floating" />
              ...
              <View className="border-t border-border pt-8 gap-4">
                ...
                >
                  <Code size={16} className="text-muted-foreground group-hover:text-sky-500" />
                  <span className="text-[10px] font-black uppercase tracking-widest">Edit Layout CSS</span>
                </Button>
              </View>
            </View>
          </View>
        ...
        >
          <TemplateStage isEditMode className="template-stage shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-white/5">
            <DesignerRenderer 
              node={activeLayout.structure as LayoutNode} 
              parentNode={null}
              contentMap={contentMap}
          ...
          <div className="absolute top-6 right-6 flex flex-row items-center gap-4 z-[100]">
            <div className="bg-card/80 border border-border rounded-full backdrop-blur-md p-1 flex flex-row items-center relative">
              ...
              >
                <Text className="text-[9px] font-black uppercase tracking-widest">Editor</Text>
              </button>
              <button 
              ...
              >
                <Text className="text-[9px] font-black uppercase tracking-widest">Preview</Text>
              </button>
            </div>
          </div>
            ...
            <div className="absolute bottom-8 right-8 flex flex-col items-end gap-4 z-[100]">
              {showMenu && (
                <div className="flex flex-col gap-2 mb-2 animate-in fade-in slide-in-from-bottom-4 duration-200">
                  <Button 
                    onClick={(e) => { e.stopPropagation(); setShowSettings(true); setShowMenu(false); }}
                    className="bg-card border border-border h-12 px-6 hover:bg-muted shadow-2xl rounded-xl"
                  >
                    <div className="flex flex-row items-center gap-3">
                      <Settings size={16} className="text-muted-foreground" />
                      <span className="text-foreground font-black uppercase tracking-widest text-[10px]">Canvas Settings</span>
                    </div>
                  </Button>
                  <Button 
                  ...
                  >
                    <div className="flex flex-row items-center gap-3">
                      ...
                      <span className="text-primary-foreground font-black uppercase tracking-widest text-[10px]">Sync Layout</span>
                    </div>
                  </Button>
                </div>
              )}
              <Button 
                ...
                className={cn(
                  ...
                  showMenu ? "bg-card border border-border rotate-90 scale-90" : "bg-sky-500 hover:bg-sky-400"
                )}
              >
          ...
          <Sheet open={showJson} onOpenChange={setShowJson}>
            <SheetContent side="right" className="w-[600px] sm:max-w-[600px] bg-card border-border p-0 flex flex-col data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right duration-300">
                ...
                <TabsContent value="css" className="flex-1 p-6">
                  <View className="flex-1 gap-4">
                    <Text className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest italic bg-sky-500/5 p-4 rounded-xl border border-sky-500/10">
                      Use standard CSS to override global presentation elements. Target .menu-item-card, .menu-price, or .slot-id-[ID].
                    </Text>
            ...
            <DialogContent className="max-w-md bg-card border-border rounded-3xl">
              ...
              <ScrollView className="max-h-[70vh]">
                <View className="gap-8 py-4 px-1">
                  <View className="gap-2">
                    <Text className="text-muted-foreground font-black uppercase text-[8px] tracking-widest leading-none mb-1">Friendly Name</Text>
                    <Input 
                  ...
                  <View className="gap-2">
                    <Text className="text-muted-foreground font-black uppercase text-[8px] tracking-widest leading-none mb-1">Entity Type</Text>
                    <div className="grid grid-cols-2 gap-2">
                      {['TEMPLATE', 'SCREEN', 'LABEL', 'PAGE'].map(t => (
                        <Button
                          ...
                          className={cn(
                            ...
                            activeLayout.type === t && "border-sky-500 bg-sky-500/5 ring-1 ring-sky-500/50 shadow-[0_0_15px_rgba(14,165,233,0.1)]"
                          )}
                        >
                          <LayoutIcon size={14} className={activeLayout.type === t ? "text-sky-500" : "text-muted-foreground"} />
                          <span className={cn("text-[9px] font-black uppercase tracking-widest", activeLayout.type === t ? "text-foreground" : "text-muted-foreground")}>{t}</span>
                        </Button>
                      ))}
                    </div>
                  </View>
                  ...
                  {activeLayout.type === 'SCREEN' && (
                    <View className="gap-4 border-t border-border pt-8 mt-2">
                      <Text className="text-sky-500 font-black uppercase text-[10px] tracking-widest">Hardware Deployment</Text>
                      <View className="gap-2">
                        <Text className="text-muted-foreground font-bold uppercase text-[8px] tracking-widest leading-none mb-1">Assigned Displays</Text>
                        <ScrollView className="max-h-40 border border-border rounded-2xl bg-muted/10 p-2">
                          {displays.length > 0 ? displays.map(d => {
                            const isAssigned = (activeLayout.config?.hardware || []).includes(d.id);
                            return (
                              <div 
                                key={d.id}
                                onClick={() => {
                                  const current = activeLayout.config?.hardware || [];
                                  const next = isAssigned ? current.filter(id => id !== d.id) : [...current, d.id];
                                  setActiveLayout(prev => ({ ...prev, config: { ...prev.config, hardware: next } }));
                                }}
                                className="flex flex-row items-center gap-3 p-3 hover:bg-sky-500/10 rounded-xl cursor-pointer transition-colors"
                              >
                                <Checkbox checked={isAssigned} />
                                <View>
                                  <Text className={cn("text-[10px] font-black uppercase", isAssigned ? "text-sky-500" : "text-foreground/70")}>{d.name}</Text>
                                  <Text className="text-[8px] text-muted-foreground font-mono">{d.id.slice(0,8)} | {d.metadata?.resolution || 'Auto'}</Text>
                                </View>
                              </div>
                            );
                          }) : (
                            <View className="p-8 items-center">
                              <MonitorPlay size={24} className="text-muted-foreground/30 mb-2" />
                              <Text className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">No Displays Found</Text>
                            </View>
                          )}
                        </ScrollView>
                      </View>
                  ...
                  <View className="gap-2 border-t border-border pt-8 mt-2">
                    <Text className="text-muted-foreground font-black uppercase text-[8px] tracking-widest leading-none mb-1">Classification Tags</Text>
                    <TagManager entityType="layout" entityId={activeLayout.id!} />
                  </View>
                </View>
              </ScrollView>
              <div className="flex flex-row justify-end mt-6">
                <Button onClick={() => setShowSettings(false)} className="bg-sky-500 h-12 px-8 rounded-2xl shadow-xl shadow-sky-500/20">
                  <span className="text-white font-black uppercase tracking-widest text-[10px]">Complete Settings</span>
                </Button>
              </div>
            </DialogContent>
          </Dialog>
          ...
          <Sheet open={!!editingNode} onOpenChange={(open) => !open && setEditingNode(null)}>
            <SheetContent side="right" className="w-full sm:w-[450px] sm:max-w-none bg-card border-l border-border p-0 flex flex-col shadow-[-20px_0_50px_rgba(0,0,0,0.5)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right duration-300">
              ...
              <ScrollView className="flex-1 p-6" key={editingNode ? (editingNode as any)._internalId : 'none'}>
                {editingNode && renderPropertyEditor(editingNode)}
              </ScrollView>

              <View className="p-6 border-t border-border bg-muted/10">
                <Button onClick={() => setEditingNode(null)} className="w-full h-14 bg-sky-500 hover:bg-sky-400 rounded-2xl shadow-xl shadow-sky-500/20 gap-3">
                  <Check size={18} className="text-white" />
                  <span className="text-white font-black uppercase tracking-widest text-xs">Apply Properties</span>
                </Button>
              </View>
            </SheetContent>
          </Sheet>
