...
export const PresentationRenderer: React.FC<Props> = ({ structure, content }) => {
  ...
  const renderSlot = (slotId: string) => {
    ...
    if (!binding) {
      return (
        <V className="bg-zinc-900/50 border border-zinc-800 border-dashed rounded-lg p-4 items-center justify-center">
          <T className="text-zinc-600 text-xs uppercase tracking-widest">{slot?.name || slotId}</T>
        </V>
      );
    }
    ...
    switch (binding.type) {
      ...
      default:
        return <T className="text-zinc-400">Unsupported Type: {binding.type}</T>;
    }
  };

  if (structure.layout === 'fullscreen') {
    return (
      <V className="flex-1 bg-black">
