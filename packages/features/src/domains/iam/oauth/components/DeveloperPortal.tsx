      ...
      <View className="flex flex-row items-center justify-between">
        <View>
          ...
          <Text className="text-muted-foreground">
            Manage your OAuth2 integrations.
          </Text>
        </View>
        <Dialog open={isRegistering} onOpenChange={setIsRegistering}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Create a new OAuth2 client for your integration.
              </DialogDescription>
            </DialogHeader>
            <View className="space-y-4 py-4">
              <View className="space-y-2">
                ...
                <Input
                  placeholder="e.g. My Inventory Sync"
                  value={newName}
      ...
      <View className="grid grid-cols-1 gap-6">
        {clients.length === 0 ? (
          ...
          clients.map((client) => (
            <Card
              key={client.id}
              className="border-border/50 shadow-lg hover:border-primary/30 transition-all rounded-[1.5rem] overflow-hidden"
            >
                  ...
                  <View className="flex flex-row gap-4">
                    <View className="flex flex-col">
                      ...
                      <span className="text-xs font-mono">
                        {(client.redirectUris as string[])[0]}
                      </span>
                    </View>
                  </View>
                  <Button
                    variant="outline"
                    size="sm"
                    className="rounded-lg text-[10px] font-bold uppercase tracking-widest gap-2"
                  >
                    <Key size={12} />
                    Rotate Secret
                  </Button>
