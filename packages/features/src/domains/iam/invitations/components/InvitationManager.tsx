    ...
    <View className="gap-8">
      ...
      <Card className="p-8 bg-card border-border gap-6">
        <View className="gap-1">
          <Text className="text-xl font-black uppercase tracking-tight">Invite Team Member</Text>
          <Text className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Add new chefs to your kitchen</Text>
        </View>
      ...
      <View className="gap-4">
        <View className="flex-row items-center gap-3 ml-1">
          ...
          <Text className="font-black uppercase tracking-widest text-sm">Pending Invitations</Text>
        </View>

        {loading ? (
          <View className="p-12 items-center"><Loader2 className="animate-spin text-primary" size={32} /></View>
        ) : invites.length === 0 ? (
          <Card className="p-12 items-center border-dashed bg-transparent opacity-40">
            ...
            <Text className="text-[10px] font-black uppercase tracking-[0.3em]">No pending invitations</Text>
          </Card>
        ) : (
          <View className="gap-3">
            {invites.map((invite) => (
              <Card key={invite.id} className={cn(
                ...
                invite.acceptedAt ? "bg-emerald-500/5 border-emerald-500/20" : "bg-card"
              )}>
                <View className="flex-row items-center gap-4">
                  <div className={cn(
                    ...
                    {invite.acceptedAt ? <CheckCircle2 size={18} className="text-emerald-500" /> : <Mail size={18} className="text-primary" />}
                  </div>
                  <View>
                    <Text className="font-black uppercase text-sm tracking-tight">{invite.email}</Text>
                    <View className="flex-row items-center gap-2">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{invite.role}</span>
                ...
                <View className="flex-row items-center gap-4">
                  {!invite.acceptedAt && (
                    <View className="items-end mr-4">
                      <Text className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">Expires</Text>
                  ...
                  {invite.acceptedAt ? (
                    <div className="px-3 py-1 bg-emerald-500/10 rounded-full">
                      <Text className="text-[8px] font-black text-emerald-600 uppercase tracking-widest">Joined</Text>
                    </div>
                  ) : (
                    <Button 
                      variant="ghost" 
