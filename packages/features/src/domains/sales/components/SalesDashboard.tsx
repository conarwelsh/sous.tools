        ...
        <Card className="border-border/50 shadow-xl rounded-3xl overflow-hidden">
          <CardContent className="p-8 flex flex-row items-center justify-between">
            <View className="space-y-1">
              ...
              <Text className="text-4xl font-black italic uppercase tracking-tighter">
                ${(metrics.totalEarned / 100).toFixed(2)}
              </Text>
            </View>
        ...
        <Card className="border-border/50 shadow-xl rounded-3xl overflow-hidden">
          <CardContent className="p-8 flex flex-row items-center justify-between">
            <View className="space-y-1">
              ...
              <Text className="text-4xl font-black italic uppercase tracking-tighter">
                ${(metrics.pendingCommissions / 100).toFixed(2)}
              </Text>
            </View>
      ...
      <View className="space-y-6">
        <View className="flex flex-row items-center justify-between">
          ...
          <View className="flex flex-row gap-2">
            <Button
              variant="outline"
              size="sm"
              className="rounded-xl gap-2 font-bold uppercase italic text-[10px]"
            >
              <Search size={14} />
              Filter Clients
            </Button>
        ...
        <View className="grid grid-cols-1 gap-4">
          {organizations.map((org) => (
            <Card
              key={org.id}
              className="border-border/50 hover:border-primary/30 transition-all rounded-2xl overflow-hidden shadow-sm"
            >
              <CardContent className="p-6 flex flex-row items-center justify-between">
                ...
                <View className="flex flex-row items-center gap-4">
                  <View className="text-right">
                    <Text className="text-[8px] font-black uppercase text-muted-foreground tracking-widest">
                      Rate
                    </Text>
                    <Text className="text-sm font-bold">
                      {org.commissionBps / 100}%
                    </Text>
                  </View>
                  <Button
                    variant="outline"
