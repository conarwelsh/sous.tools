  ...
  <BaseLayout preview={`[Support ${type}] ${subject}`}>
    ...
    <Section className="bg-zinc-50 p-6 rounded-2xl border border-zinc-100 mb-8">
      <div className="flex justify-between items-center mb-4">
        <div
          className={`px-2 py-0.5 rounded text-[8px] font-black uppercase bg-zinc-900 text-white`}
        >
          {type}
        </div>
        {priority && (
      ...
      <Text className="text-xs text-zinc-600 leading-relaxed mb-4">
        {description}
      </Text>
      ...
      {githubIssueUrl && (
        <Link
          href={githubIssueUrl}
          className="text-blue-600 text-[10px] font-bold underline"
        >
          View GitHub Issue
        </Link>
      )}
    ...
    <Section className="p-4 bg-zinc-100 rounded-xl">
      <Text className="text-[10px] font-bold text-zinc-500 uppercase mb-2">
