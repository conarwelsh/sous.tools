...
interface LowStockEmailProps {
  items: Array<{ name: string; current: number; threshold: number; unit: string }>;
}
...
export const LowStockEmail = ({ items }: LowStockEmailProps) => (
  <BaseLayout preview="Inventory Alert: Low Stock Items">
    ...
    <Text className="text-zinc-600 text-sm leading-relaxed mb-6">
      The following items have fallen below their par levels and may need to be reordered soon.
    </Text>
    <Section className="mb-8">
      {items.map((item, idx) => (
        <div key={idx} className="bg-rose-50 border border-rose-100 p-4 rounded-xl mb-3">
