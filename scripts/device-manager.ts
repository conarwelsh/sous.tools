...
const IS_WSL = fs.readFileSync('/proc/version', 'utf8').toLowerCase().includes('microsoft');
...
async function main() {
  ...
  if (serial) {
    ...
    if (IS_WSL) {
      try {
        execSync(`${ADB} connect ${WIN_IP}:${serial.split('-')[1]}`, { stdio: 'ignore' });
      } catch (e) {}
    }
    console.log(serial);
    process.exit(0);
  ...
  if (IS_WSL) {
    ...
    spawn(EMULATOR, ['-avd', AVD_NAME], { detached: true, stdio: 'ignore' }).unref();
  }

  // 3. Wait for boot and discover serial
  console.error(`⏳ Waiting for ${AVD_NAME} to register and finish booting (max 120s)...`);
  let attempts = 0;
...
async function findSerialByAvd(targetAvd: string): Promise<string | null> {
  try {
    ...
    const devicesOutput = execSync(`${ADB} devices`, { timeout: 5000, stdio: ['pipe', 'pipe', 'ignore'] }).toString();
    const lines = devicesOutput.split('\n').filter(l => l.includes('\tdevice'));
    
    for (const line of lines) {
      const serial = line.split('\t')[0].trim();
      try {
        const avdName = execSync(`${ADB} -s ${serial} shell getprop ro.boot.qemu.avd_name`, { timeout: 3000, stdio: ['pipe', 'pipe', 'ignore'] }).toString().trim();
        if (avdName === targetAvd) return serial;
        
        // Fallback to model check
        const model = execSync(`${ADB} -s ${serial} shell getprop ro.product.model`, { timeout: 3000, stdio: ['pipe', 'pipe', 'ignore'] }).toString().trim();
        if (model.toLowerCase().includes(targetAvd.toLowerCase().replace(/_/g, ' '))) return serial;
      } catch (e) {
