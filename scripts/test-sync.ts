...
const squareDriverPath = path.resolve('apps/api/src/domains/integrations/drivers/square.driver.ts');
...
async function test() {
  ...
  if (!cfg.square.accessToken) {
    ...
    logger.info('Current square config:', JSON.stringify(cfg.square, null, 2));
    process.exit(1);
  }

  logger.info(`🧪 Testing Square Integration (${config.square.environment})...`);
  logger.info(`📱 Application ID: ${config.square.applicationId || 'MISSING'}`);
  logger.info(`🔑 Token Status: ${config.square.accessToken ? 'PRESENT' : 'MISSING'}`);
  ...
  try {
    ...
    logger.info(`✅ Found ${categories.length} categories and ${items.length} items.`);
    ...
    for (const item of items) {
      if (item.categoryId) {
        ...
        if (cat) {
          logger.info(`🔗 Item "${item.name}" is correctly linked to category "${cat.name}"`);
          linkedCount++;
        } else {
          logger.warn(`⚠️ Item "${item.name}" has categoryId ${item.categoryId} but category not found in list`);
        }
      } else {
    ...
    if (linkedCount === items.length && items.length > 0) {
      logger.info('🚀 SUCCESS: All items are correctly associated with categories!');
    } else if (items.length === 0) {
      logger.error('❌ FAIL: No items found in catalog');
    } else {
      logger.error(`❌ FAIL: Only ${linkedCount}/${items.length} items were correctly linked`);
    }
