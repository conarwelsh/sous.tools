...
async function main() {
  ...
  try {
    ...
    if (
      data.action === "fix" &&
      data.filesToModify &&
      data.filesToModify.length > 0
    ) {
      const branch =
        data.branchName || `gemini/fix-${issueNumber}-${Date.now()}`;
      ...
      execSync(
        `git commit -m "${data.prTitle || `fix: Gemini action for #${issueNumber}`}"`,
      );
      ...
      const prRes = await fetch(
        `https://api.github.com/repos/${repoFull}/pulls`,
        {
          method: "POST",
          headers: {
    ...
    await postComment(
      ...
      issueNumber!,
      githubToken,
      `❌ **Error encountered during action:**\n\n\`\`\`\n${error}\n\`\`\``,
    );
    process.exit(1);
  }
}
