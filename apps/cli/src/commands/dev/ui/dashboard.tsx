...
export const Dashboard: React.FC<Props> = ({ manager }) => {
  ...
  useInput((input, key) => {
    ...
    if (key.ctrl && (input === 'R' || (key.shift && input === 'r'))) {
      ...
      exec('pnpm sous db reset', (error, stdout, stderr) => {
        if (stdout)
          setTerminalOutput((prev) => [...prev, ...stdout.trim().split('\n')]);
        if (stderr)
  ...
  const handleGeminiSubmit = (value: string) => {
    ...
    exec(`gemini "${value}"`, (error, stdout, stderr) => {
      if (stdout)
        setGeminiOutput((prev) => [...prev, ...stdout.trim().split('\n')]);
      if (stderr)
      ...
      if (error)
        setGeminiOutput((prev) => [...prev, `[FAIL] ${error.message}`]);
      setGeminiCommand('');
      setIsGeminiMode(false);
    });
  };
    ...
    >
      ...
      <Box flexGrow={1}>
        ...
        <Box
          ...
          borderStyle="round"
          borderColor={
            activeTab === 'services' && selectedApp?.status === 'error'
              ? BRAND_BLUE
              : activeTab === 'gemini'
                ? BRAND_PURPLE
                : DARK_GRAY
        ...
        >
              ...
              <Box flexDirection="column" flexGrow={1} overflow="hidden">
                {terminalOutput
                  .slice(
                    Math.max(
                      0,
                      terminalOutput.length - visibleLines - scrollOffset,
                    ),
              ...
              <Box flexDirection="column" flexGrow={1} overflow="hidden">
                {geminiOutput
                  .slice(
                    Math.max(
                      0,
                      geminiOutput.length - visibleLines - scrollOffset,
                    ),
                  ...
                  .map((line, i) => (
                    <Text
                      key={i}
                      color={line.startsWith('👨‍🍳') ? BRAND_PURPLE : '#d1d5db'}
                    >
                      {line}
                    </Text>
                  ))}
          ...
          {activeTab === 'infra' && (
            <Box flexDirection="column" flexGrow={1}>
              ...
              >
                ...
                <Box flexDirection="column">
                  ...
                  <Box justifyContent="space-between">
                    ...
                    <Text>
                      {formatMem(systemMetrics.mem)} /{' '}
                      {formatMem(systemMetrics.totalMem)}
                    </Text>
                  </Box>
                  <Box justifyContent="space-between">
                    <Text>Uptime</Text>
                    <Text>
                      {(systemMetrics.uptime / 3600).toFixed(1)} hours
                    </Text>
                  </Box>
                </Box>
              </Box>
              ...
              <Box marginTop={1} flexDirection="column">
                ...
                >
                  <Text color="gray">
                    Metrics for staging/production will appear here
                  </Text>
                </Box>
              </Box>
            </Box>
          ...
          {activeTab === 'rpi' && (
            <Box flexGrow={1} alignItems="center" justifyContent="center">
              <Text color="gray">
                Remote Raspberry Pi: 192.168.1.10 (Online)
              </Text>
            </Box>
          )}
        </Box>
      ...
      <Box flexDirection="column" height={6}>
        <Box
          borderStyle="round"
          borderColor={
            isCommandMode ? BRAND_BLUE : isGeminiMode ? BRAND_PURPLE : DARK_GRAY
          }
          paddingX={1}
          flexGrow={1}
        >
          <Box justifyContent="space-between">
            ...
            {(isCommandMode || isGeminiMode) && (
              <Text color={isGeminiMode ? BRAND_PURPLE : BRAND_BLUE} italic>
                PROMPT ACTIVE
              </Text>
            )}
          </Box>
        ...
        <Box justifyContent="space-between" paddingX={1}>
          <Text color="gray" dimColor>
            [Tab] View [↑/↓] Scroll [Enter] Toggle [r] Restart [c] Clear [/]
            Filter [:] Cmd [;] Gemini [q] Quit
          </Text>
        </Box>
      </Box>
    </Box>
