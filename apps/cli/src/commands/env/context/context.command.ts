...
@Command({
  ...
  subCommands: [WhoamiCommand, SwitchEnvCommand, LoginAsCommand],
})
export class ContextCommand extends CommandRunner {
  async run(passedParam: string[]): Promise<void> {
    if (
      passedParam.length > 0 &&
      ['whoami', 'switch-env', 'login-as'].includes(passedParam[0])
    ) {
      return;
    }
    logger.info('Please specify a subcommand: whoami, switch-env, login-as');
  }
}
