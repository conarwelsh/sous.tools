...
@Command({
  ...
  subCommands: [ConfigCommand, LogsCommand, BrandingCommand, EnvExportCommand],
})
export class EnvCommand extends CommandRunner {
  async run(passedParam: string[]): Promise<void> {
    if (
      passedParam.length > 0 &&
      ['config', 'logs', 'branding', 'export'].includes(passedParam[0])
    ) {
      return;
    }
    logger.info('Please specify a subcommand: config, logs, branding, export');
  }
}
