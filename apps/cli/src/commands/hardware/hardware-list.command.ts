...
export class HardwareListCommand extends CommandRunner {
  async run(): Promise<void> {
    ...
    console.log(
      `${chalk.bold('ID'.padEnd(10))} ${chalk.bold('NAME'.padEnd(20))} ${chalk.bold('STATUS'.padEnd(10))} ${chalk.bold('VERSION')}`,
    );
    console.log(
      `${'hw_123'.padEnd(10)} ${'Kitchen Pi'.padEnd(20)} ${chalk.green('ONLINE'.padEnd(10))} v0.1.0`,
    );
    console.log(
      `${'hw_456'.padEnd(10)} ${'Bar Display'.padEnd(20)} ${chalk.gray('OFFLINE'.padEnd(10))} v0.1.0`,
    );
    console.log('='.repeat(50) + '\n');
  }
}
