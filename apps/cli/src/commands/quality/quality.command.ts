...
@Command({
  ...
  subCommands: [TestCommand, CheckCommand, ForgeCommand],
})
export class QualityCommand extends CommandRunner {
  async run(passedParam: string[]): Promise<void> {
    if (passedParam.length > 0 && ['test', 'check', 'forge'].includes(passedParam[0])) {
      return;
    }
    console.log('Please specify a subcommand: test, check, forge');
  }
}
