...
export class AccountingService {
  ...
  async recordEntry(data: typeof generalLedger.$inferInsert) {
    const result = await this.dbService.db.insert(generalLedger).values(data).returning();
    return result[0];
  }

  async getLedger(organizationId: string) {
    return this.dbService.db.select().from(generalLedger).where(eq(generalLedger.organizationId, organizationId));
  }
