...
import { View, Text, Button, Card, GoogleDriveLogo, ScrollView, InlineInput, InlineTextArea, cn } from "@sous/ui";
import { ChefHat, Scale, ArrowLeft, Loader2, Sparkles, Clock, ListChecks, Trash2, Save, Plus, Play } from "lucide-react";
...
const UPDATE_RECIPE = gql`
  mutation UpdateRecipe($orgId: String!, $id: String!, $input: UpdateRecipeInput!) {
  ...
  if (loading || !formData) {
    return (
      <View className="flex-1 bg-background p-8 items-center justify-center">
        ...
        <Text className="text-muted-foreground font-black uppercase text-xs tracking-widest">Loading Recipe IP...</Text>
      </View>
    );
  }
    ...
    <View className="flex-1 bg-background p-8 relative">
          ...
          <View className="flex-row justify-between items-center mb-8">
            ...
            <div className="flex flex-row gap-3">
              ...
              >
                ...
                <Text className="text-white font-black uppercase text-[10px] tracking-widest">Cook Mode</Text>
              </Button>
              <Button 
                variant="ghost" 
              ...
              >
                <Trash2 size={16} />
                <Text className="text-[10px] font-black uppercase tracking-widest">Wipe IP</Text>
              </Button>
            </div>
          </View>
            ...
            <View className="flex-1">
              ...
              <div className="flex flex-row gap-4 items-center">
                <div className="flex items-center gap-3 text-muted-foreground bg-muted/30 px-4 py-2 rounded-xl border border-border/50">
                  ...
                  <div className="flex flex-row items-center gap-1">
                    ...
                    <InlineInput 
                      ...
                      onValueChange={(val) => handleFieldChange({ yieldAmount: val })}
                      className="w-12 text-center font-mono font-bold"
                    />
                    <InlineInput 
                      value={formData.yieldUnit || '---'}
                      onValueChange={(val) => handleFieldChange({ yieldUnit: val })}
                ...
                {!formData.ingredients.length && (
                  ...
                  >
                    {isIngesting ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}
                    <Text className="font-black uppercase text-[10px] tracking-widest">Parse with AI</Text>
                  </Button>
                )}
              </div>
            </View>
          ...
          <View className="lg:col-span-4">
            <View className="flex-row items-center justify-between mb-8">
              <View className="flex-row items-center gap-3">
                ...
                <Text className="font-black uppercase text-xs tracking-[0.2em]">Components</Text>
              </View>
              <Button onClick={addIngredient} variant="ghost" className="h-8 w-8 p-0 rounded-full hover:bg-sky-500/10 text-sky-500">
                <Plus size={18} />
              </Button>
            </View>
            ...
            <Card className="bg-card border-border/50 divide-y divide-border/30 rounded-[2rem] overflow-hidden shadow-2xl shadow-black/20">
              {formData.ingredients.map((ri: any, idx: number) => (
                <View key={ri.id} className="p-5 flex-row justify-between items-center group">
                  <InlineInput 
                    value={ri.ingredient.name}
                    onValueChange={(val) => handleIngredientChange(idx, { ingredient: { ...ri.ingredient, name: val } })}
                  ...
                  <div className="flex flex-row items-center gap-2 bg-muted/20 p-1 rounded-lg border border-border/50 opacity-60 group-hover:opacity-100 transition-opacity">
                    <InlineInput 
                      ...
                      onValueChange={(val) => handleIngredientChange(idx, { amount: val })}
              ...
              {!formData.ingredients.length && (
                <View className="p-12 items-center justify-center opacity-30">
                  <Text className="text-[10px] font-black uppercase tracking-widest">No Ingredients Yet</Text>
                </View>
              )}
            </Card>
          </View>
          ...
          <View className="lg:col-span-8">
            <View className="flex-row items-center justify-between mb-8">
              <View className="flex-row items-center gap-3">
                ...
                <Text className="font-black uppercase text-xs tracking-[0.2em]">Execution Protocol</Text>
              </View>
              <Button onClick={addStep} variant="ghost" className="h-8 w-8 p-0 rounded-full hover:bg-amber-500/10 text-amber-500">
                <Plus size={18} />
              </Button>
            </View>

            <View className="gap-8">
              {formData.steps.sort((a: any, b: any) => a.order - b.order).map((step: any, idx: number) => (
                <View key={step.id} className="flex-row gap-8 group">
                  <View className="items-center">
                    <View className="w-12 h-12 rounded-2xl bg-muted/30 flex items-center justify-center border border-border/50 group-hover:border-amber-500/30 group-hover:bg-amber-500/5 transition-all">
                      <Text className="text-muted-foreground font-black text-xs group-hover:text-amber-500">{idx + 1}</Text>
                    </View>
                  ...
                  <View className="flex-1 pb-4">
                    <Card className="p-6 bg-card border-border/50 rounded-3xl hover:border-amber-500/20 transition-all shadow-xl shadow-black/5 relative">
                      <InlineTextArea 
                        ...
                        onValueChange={(val) => handleStepChange(idx, { instruction: val })}
                      ...
                      <div className="mt-4 flex flex-row items-center gap-4">
                        <div className="flex flex-row items-center gap-2 text-muted-foreground/60 bg-muted/20 px-3 py-1.5 rounded-xl border border-border/50">
                          ...
                          <div className="flex flex-row items-center gap-1">
                            <InlineInput 
                              value={step.timerDuration ? String(Math.floor(step.timerDuration / 60)) : ""}
                              placeholder="0"
                              onValueChange={(val) => handleStepChange(idx, { timerDuration: parseInt(val) * 60 || null })}
              ...
              {!formData.steps.length && (
                <Card className="p-20 border-dashed border-border/50 items-center justify-center rounded-[3rem] bg-muted/5">
                   <Text className="text-muted-foreground font-black uppercase text-[10px] tracking-[0.2em]">Empty Protocol</Text>
                </Card>
              )}
            </View>
          </View>
      ...
      {isDirty && (
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[100] animate-in fade-in slide-in-from-bottom-8 duration-500">
          ...
          >
            {isSaving ? <Loader2 size={24} className="animate-spin" /> : <Save size={24} className="text-white" />}
            <View>
              <Text className="text-white font-black uppercase text-sm tracking-tighter leading-none mb-0.5">Commit Changes</Text>
              <Text className="text-sky-100 text-[8px] font-black uppercase tracking-[0.2em] leading-none">Save to Repository</Text>
            </View>
          </Button>
        </div>
      )}
