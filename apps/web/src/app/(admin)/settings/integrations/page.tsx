    ...
    <View className="flex-1 bg-background p-8">
      ...
      {status === "success" && (
        <View className="mb-8 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex-row items-center justify-between">
          ...
          >
            <Text className="text-emerald-500/50 font-bold uppercase text-[10px]">
              Dismiss
            </Text>
          </button>
        </View>
        ...
        <View className="mb-8 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex-row items-center justify-between">
          <View className="flex-row items-center gap-3">
            ...
            <Text className="text-rose-500 font-bold uppercase text-xs tracking-widest">
              Failed to connect{" "}
              {provider === "google-drive" ? "Google Drive" : provider}. Please
              try again.
            </Text>
          </View>
          <button
          ...
          >
            <Text className="text-rose-500/50 font-bold uppercase text-[10px]">
              Dismiss
            </Text>
          </button>
        </View>
      ...
      {error && (
        <View className="mb-8 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex-row items-center justify-between">
          ...
          <button onClick={() => setError(null)}>
            <Text className="text-rose-500/50 hover:text-rose-500 font-bold uppercase text-[10px]">
              Dismiss
            </Text>
          </button>
        </View>
        ...
        {connectors.map((c) => {
          ...
          const isConnected =
            ...
            (integration.isActive === true ||
              ...
              integration.isActive === "t" ||
              integration.is_active === "t");

          return (
            <Card
              key={c.id}
              className="p-8 bg-card border-border border-2 hover:border-primary/50 transition-all flex flex-col group"
            >
              ...
              {isConnected ? (
                <View className="mt-auto gap-3">
                  ...
                  >
                    <View className="flex-row items-center gap-2">
                      <Trash2
                        size={16}
                        className="text-muted-foreground group-hover/disconnect:text-destructive"
                      />
                ...
                >
                  {isSubmitting && connecting === null ? (
                    <Loader2
                      className="animate-spin text-foreground group-hover:text-primary-foreground"
                      size={18}
                    />
                  ) : (
        ...
        <DialogContent className="sm:max-w-[425px] bg-background border-border">
          ...
          <div className="py-6">
            <Text className="text-sm text-muted-foreground leading-relaxed">
              ...
              <span className="text-foreground font-bold">
                {connectors.find((c) => c.id === disconnectingProvider)?.name}
              </span>
              ? This will stop all automated data synchronization. Historical
              data will remain in your system but will no longer be updated.
            </Text>
          </div>
          ...
          <DialogFooter className="gap-3">
            ...
            >
              {isDisconnecting ? (
                <Loader2
                  className="animate-spin text-destructive-foreground"
                  size={20}
                />
              ) : (
