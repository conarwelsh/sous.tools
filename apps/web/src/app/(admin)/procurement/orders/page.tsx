...
import { View, Text, Button, Card, Input, ScrollView, cn, Checkbox } from "@sous/ui";
import { 
  ShoppingBag, 
  Plus, 
  Search, 
...
const UPDATE_LIST_ITEM = gql`
  mutation UpdateShoppingListItem($orgId: String!, $id: ID!, $input: UpdateShoppingListItemInput!) {
...
export default function OrderManagerPage() {
  ...
  const { data, loading, refetch } = useQuery<ProcurementData>(GET_PROCUREMENT_DATA, {
    variables: { orgId },
    skip: !orgId,
  });

  const [addToList] = useMutation(ADD_TO_LIST, { onCompleted: () => refetch() });
  const [updateItem] = useMutation(UPDATE_LIST_ITEM, { onCompleted: () => refetch() });
      ...
      <View className="flex-row justify-between items-end mb-12">
        ...
        <View className="flex-row gap-3">
           <div className="flex bg-muted/50 rounded-2xl p-1 border border-border">
              <Button variant="ghost" className="h-10 px-6 rounded-xl bg-background shadow-sm">
                <Text className="text-[10px] font-black uppercase tracking-widest text-primary">Living List</Text>
              </Button>
              <Button variant="ghost" className="h-10 px-6 rounded-xl">
                <Text className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Order History</Text>
              </Button>
           </div>
        </View>
      </View>
        ...
        <View className="flex-[3] gap-8">
                ...
                {filteredIngredients.map((ing: any) => (
                  ...
                  >
                    <View className="flex-row items-center gap-4">
                      ...
                      <Text className="font-black uppercase tracking-tight text-foreground">{ing.name}</Text>
                    </View>
                    <Text className="text-[10px] font-black uppercase text-muted-foreground bg-muted px-3 py-1 rounded-full">{ing.baseUnit}</Text>
                  </div>
                ))}
              </Card>
            )}
          ...
          {loading ? (
            ...
            <Card className="p-20 bg-card border-border border-dashed items-center justify-center rounded-[2.5rem]">
              ...
              <Text className="text-muted-foreground font-black uppercase text-xs tracking-widest mb-2">Everything looks good</Text>
              <Text className="text-muted-foreground/60 text-sm max-w-xs text-center mb-8">
                Your living order list is empty. Add items as you notice they are low, or wait for system suggestions.
              </Text>
            </Card>
          ) : (
                    ...
                    <View className="flex-row items-center justify-between px-2">
                      <View className="flex-row items-center gap-4">
                        <div className={cn(
                          ...
                          isUnassigned ? "bg-muted/20 border-border" : "bg-sky-500/10 border-sky-500/20 shadow-[0_0_20px_rgba(14,165,233,0.1)]"
                        )}>
                          <Truck size={24} className={isUnassigned ? "text-muted-foreground" : "text-sky-500"} />
                        </div>
                        <View>
                          ...
                          {!isUnassigned && (
                            <View className="flex-row items-center gap-3">
                              ...
                              <Text className="text-muted-foreground/30 text-[10px]">•</Text>
                      ...
                      {!isUnassigned && (
                        ...
                        >
                          <Check size={18} className="text-primary-foreground group-hover:scale-110 transition-transform" />
                        ...
                        >
                          <View className="flex-row items-center gap-4 flex-1">
                            <div className="w-10 h-10 rounded-xl bg-muted/30 flex items-center justify-center border border-border group-hover/item:bg-primary/5 transition-colors">
                              <Package size={18} className="text-muted-foreground group-hover/item:text-primary transition-colors" />
                            </div>
                            <View className="flex-1">
                              <Text className="font-black uppercase text-sm tracking-tight text-foreground">{item.ingredient.name}</Text>
                              <View className="flex-row items-center gap-3">
                                ...
                                {item.source === 'system_suggestion' && (
                                  <div className="flex flex-row items-center gap-1 text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20">
                                    ...
                                    <Text className="text-[7px] font-black uppercase tracking-widest">Suggested</Text>
                                  </div>
                                )}
                              </View>
                            </View>
                          ...
                          <View className="flex-row items-center gap-6">
                            <View className="items-end">
                               <div className="flex flex-row items-center gap-2 bg-muted/20 p-1 rounded-xl border border-border/50">
                                  ...
                                  />
                                  <Text className="text-[10px] font-black uppercase text-muted-foreground pr-3 border-l border-border/50 pl-2">{item.unit}</Text>
                               </div>
                            </View>
                            
                            <select 
                              value={item.preferredSupplier?.id || ""}
                              onChange={(e) => updateItem({ variables: { orgId, id: item.id, input: { preferredSupplierId: e.target.value || null } } })}
                            ...
                            <Button variant="ghost" className="h-10 w-10 p-0 rounded-full opacity-0 group-hover/item:opacity-100 transition-opacity hover:bg-red-500/10 hover:text-red-500">
                              <Trash2 size={16} />
                            </Button>
                          </View>
                        </Card>
        ...
        <View className="flex-1 flex flex-col gap-8">
          <Card className="p-6 bg-card border-border rounded-3xl shadow-2xl">
            ...
            <View className="gap-6">
               <View className="gap-2">
                  <Text className="text-[10px] font-black uppercase text-muted-foreground leading-tight">Cutoff Reminders</Text>
                  <Card className="p-4 bg-muted/20 border-border border-dashed rounded-2xl items-center justify-center">
                    ...
                    <Text className="text-[9px] font-black uppercase text-muted-foreground/40 tracking-widest text-center">No orders reaching cutoff in next 4h</Text>
                  </Card>
               </View>

               <View className="gap-2">
                  <Text className="text-[10px] font-black uppercase text-muted-foreground leading-tight">Supplier Schedule</Text>
                  <View className="gap-2">
                    {suppliers.slice(0, 3).map(s => (
                      <div key={s.id} className="flex flex-row items-center justify-between p-3 bg-muted/10 border border-border/30 rounded-xl">
                        <Text className="text-[9px] font-black uppercase text-foreground/70">{s.name}</Text>
                        <Text className="text-[8px] font-black uppercase text-sky-500">{getNextDelivery(s.deliveryDays)}</Text>
                      </div>
                    ))}
                  </View>
               </View>
          ...
          <Card className="p-6 bg-sky-500 border-sky-400 rounded-3xl shadow-2xl shadow-sky-500/20">
            <View className="flex-row items-center gap-3 mb-4">
              ...
              <Text className="text-white font-black uppercase text-[10px] tracking-widest">New Supplier</Text>
            </View>
            <Text className="text-white/80 text-xs mb-6 font-medium leading-relaxed">Expand your network to optimize pricing and availability.</Text>
            <Button variant="ghost" className="w-full bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-xl h-10">
              <span className="font-black uppercase text-[10px] tracking-widest">Add Vendor</span>
            </Button>
          </Card>
        </View>
      </View>
