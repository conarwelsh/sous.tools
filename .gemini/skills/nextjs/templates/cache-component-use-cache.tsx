...
export async function getExpensiveData(id: string) {
  ...
  const data = await fetch(`https://api.example.com/items/${id}`).then((r) =>
    r.json(),
  );
...
export async function getPopularPosts() {
  const posts = await fetch("https://api.example.com/posts/popular").then((r) =>
    r.json(),
  );
...
export async function getRealtimeMetrics() {
  const metrics = await fetch("https://api.example.com/metrics/realtime").then(
    (r) => r.json(),
  );
...
export async function MixedCachingComponent() {
  ...
  const freshData = await fetch("https://api.example.com/fresh-data").then(
    (r) => r.json(),
  );
