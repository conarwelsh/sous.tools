...
- **Dynamic Context (Request/Execution Level):**
  ...
  - _Example:_ `@sous/web` starts a request. It sets the storage context. When `@sous/web` calls a function in `@sous/config` or `@sous/ui`, those packages (using their own namespaced loggers) will automatically include the `trace_id` from the active storage context.
