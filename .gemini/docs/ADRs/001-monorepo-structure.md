...
4. **Scaffold Core Packages:** Initialize `@sous/config` and `@sous/logger` as first-order dependencies.
5. **Setup Apps:** Create basic Next.js and NestJS skeletons in `apps/`.
6. **CI/CD Integration:** Configure GitHub Actions to run `turbo run build lint test` on every PR.
