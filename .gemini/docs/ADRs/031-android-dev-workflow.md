...
The codebase resides in WSL2 (Ubuntu) for performance and consistency with the backend/CLI environment. However, Android Emulators and physical device USB drivers perform significantly better or are only available on the Windows host. We need a way to build Android apps (Capacitor and Wear OS) inside WSL2 while debugging on Windows-hosted emulators or physical devices.

### Identified Problems:

1. **Binary/Path Collision**: Capacitor discovery logic searches for "Android Studio" but finds the Windows directory instead of a binary, throwing `Permission Denied`.
...
1.  **Build Environment:** All compilation (Gradle, Capacitor) occurs inside WSL2 to avoid cross-OS filesystem overhead.
